...
/////// Normals:
                float2 node_1128 = i.uv0;
                float3 normalLocal = UnpackNormal(tex2D(_Normals,TRANSFORM_TEX(node_1128.rg, _Normals))).rgb;
...
////// Lighting:
                ...
                float3 finalColor = (attenuation*_LightColor0.rgb*((tex2D(_Diffuse,TRANSFORM_TEX(node_1128.rg, _Diffuse)).rgb*_Color.rgb*floor(max(0,dot(lightDirection,node_41)) * _Bands) / (_Bands - 1))+UNITY_LIGHTMODEL_AMBIENT.rgb+floor(pow(max(0,dot(node_41,halfDirection)),exp2(((_Gloss*10.0)+1.0))) * _Bands) / (_Bands - 1)));
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
/////// Normals:
                float2 node_1129 = i.uv0;
                float3 normalLocal = UnpackNormal(tex2D(_Normals,TRANSFORM_TEX(node_1129.rg, _Normals))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
////// Lighting:
                float attenuation = LIGHT_ATTENUATION(i);
                float3 node_41 = normalDirection;
                float3 finalColor = (attenuation*_LightColor0.rgb*((tex2D(_Diffuse,TRANSFORM_TEX(node_1129.rg, _Diffuse)).rgb*_Color.rgb*floor(max(0,dot(lightDirection,node_41)) * _Bands) / (_Bands - 1))+UNITY_LIGHTMODEL_AMBIENT.rgb+floor(pow(max(0,dot(node_41,halfDirection)),exp2(((_Gloss*10.0)+1.0))) * _Bands) / (_Bands - 1)));
/// Final Color:
